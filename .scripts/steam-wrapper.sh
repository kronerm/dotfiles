...
if [ "${GAMESCOPE_DISABLED:-"0"}" != "1" ]; then
  ...
  selected_display_resolution="$(echo -e "$available_modes_with_refresh_rates_json" | jq -r 'keys[]' | sort -hr | rofi -dmenu)"
  selected_display_refresh_rate="$(echo -e "$available_modes_with_refresh_rates_json" | jq -r ".\"$selected_display_resolution\"[]" | sort -hr | rofi -dmenu)"
